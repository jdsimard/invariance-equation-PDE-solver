...
- the resulting mappings have the property that

$$\pi(\omega) - \hat{\pi}(\omega) = \sum_{i = d+1}^{\infty} \sum_{|m| = i} \Pi_m \omega^m,$$

so the solution approximation error is $\theta(|\omega|^{d+1})$ and can be improved with each increment in the desired total order of the approximation
