...
%   Detailed explanation goes here
  ...
  if symbolic_pde_solver.internal_utils.input_verification.consistent_coeff_matrix_and_props(size(coeffs,1),size(coeffs,2),nu,n,known_degree) == false
    ...
    pi_known = 0;
    if known_degree > 0
      num_new_monomials = symbolic_pde_solver.internal_utils.multiset.num_monomials_of_order_one_to_d(nu,known_degree);
      known_monomials = sym(zeros([num_new_monomials,1]));
      for i1 = 1 : 1 : known_degree
        % calculate the index range for the new monomials
        index_first = symbolic_pde_solver.internal_utils.multiset.num_monomials_of_order_one_to_d(nu,i1-1) + 1;
        index_last = symbolic_pde_solver.internal_utils.multiset.num_monomials_of_order_one_to_d(nu,i1);
        % now get the monomials
        known_monomials(index_first:index_last,1) = symbolic_pde_solver.internal_utils.symbolic.get_monomial_terms_of_total_order(w,i1);
      end
    ...
    if symbolic_pde_solver.internal_utils.file_io.save_solution_files(nu,m,n,known_degree,new_known_coeffs,filename_properties,filename_coefficients) == false
      ...
      % return false
      success_state = false;
    elseif symbolic_pde_solver.internal_utils.file_io.save_functions_defining_pde(s,ell,f,filename_symfuns) == false
