...
%   Detailed explanation goes here
  ...
  if fn_auxiliary.save_functions_defining_pde(fn_s,fn_ell,fn_f,filename_symfuns) == false
    % the functions are not consistent, don't save the files, return false
    consistency_state = false;
    return;
  end
  [s,ell,f,w,u,x,nu,m,n] = fn_auxiliary.load_functions_defining_pde(filename_symfuns);

  if fn_auxiliary.save_solution_files(nu,m,n,0,zeros(length(w),0),filename_properties,filename_coefficients) == false
